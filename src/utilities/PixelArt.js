  ...
  useEffect(() => {
    ...
    var Teamkill = function () {
      ...
      this.run = function () {
        ...
        patternEngine.font = "italic bold "
          .concat(fontSize)
          .concat("px ")
          .concat(font);
      ...
      var prepareText = function () {
        ...
        patternEngine.fillText(
          text,
          window.innerWidth / 2 - m.width / 2,
          window.innerHeight / 2 - fontSize / 2
        );

        var imgData = patternEngine.getImageData(
          0,
          0,
          window.innerWidth,
          window.innerHeight
          ...
          for (var x = 0; x < window.innerWidth; x += sensitivity) {
            if (imgData.data[4 * (y * window.innerWidth + x)] != 0) {
              pixelStops.push({
                ...
                color:
                  "hsl(" +
                  ((360 / textBound.width) * x - textBound.left) +
                  ", 50%, 50%)",
              });
            }
          }
      ...
      var generatePixels = function () {
        ...
        for (
          var i = 0, max = Math.ceil(pixelStops.length * pixelMultiplier);
          i < max;
          i++
        ) {
          ...
          var speedX =
            0.5 - Math.random() < 0 ? split * speed : 0 - split * speed;
          var speedY =
            0.5 - Math.random() < 0
              ? (1 - split) * speed
              : 0 - (1 - split) * speed;
      ...
      var drawHighlight = function () {
        ...
        if (highlightTicker > 0 && highlightTicker <= highlightTickerMax) {
          ...
          var left =
            textBound.left -
            20 +
            ((textBound.width + 40) / highlightTickerMax) * highlightTicker;
