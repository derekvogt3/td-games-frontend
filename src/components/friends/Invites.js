...
function Invites({ currentUser, friend, showAlert }) {
  ...
  useEffect(() => {
    fetch(`/friends_relation?user_id=${currentUser.id}&friend_id=${friend.id}`)
      .then((res) => res.json())
    ...
    const intervalId = setInterval(() => {
      fetch(
        `/friends_relation?user_id=${currentUser.id}&friend_id=${friend.id}`
      )
        .then((res) => res.json())
        .then((data) => setInvite(data));
    }, 2000);
  ...
  function cancelInvite(e) {
    ...
    fetch(
      `/delete_relations?user_id=${currentUser.id}&friend_id=${friend.id}&method=delete`,
      {
        method: "DELETE",
      }
    )
  ...
  function declineInvite(e) {
    e.target.style.pointerEvents = "none";
    fetch(
      `/delete_relations?user_id=${currentUser.id}&friend_id=${friend.id}&method=decline`,
      {
        method: "DELETE",
      }
    )
  ...
  function acceptInvite(e) {
    e.target.style.pointerEvents = "none";
    fetch(`/accept_relations?user_id=${currentUser.id}&friend_id=${friend.id}`)
      .then((res) => res.json())
      .catch(console.error);
  }
